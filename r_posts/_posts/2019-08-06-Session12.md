...
After modifying and debugging all the hardware issues with the FET board, I eventually managed to get all the keys to work. Now that the keys are working, I needed to build a base for the piano. After coming with multiple designs, I chose and 3D printed one which can hold the solenoids without shorting them. However, the CURA slicer wasn't working well, so I decided to use the Prusa Slicer. 
